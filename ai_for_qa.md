...
## Как пользоваться агентами для написания автотестов
...
5. Лучше начать новую сессию если в рамках сессии поставленная задача решена и новая задача имеет мало пересечений с выполненной. Большой контекст может путать. Превышение контекста модели приводит к галюцинациям, забывает важное, лучше избегать, на максимальный контекст модели -15% уже может запуститься механизм сжатия.
...
10. Для контроля изменений использовать git, коммитить изменения перед задачей, после выполнения задачи смотрим изменненные файлы, новый и измененный код. Коммитим если все устраивает.
11. С одной итерации идеальный и рабочий код не всегда получается - это нормально, не нужно откатывать если конечно не совсем все плохо вышло, просим исправить что не так, просим прогнать тесты, сделать ревью "Сделай ревью, используй лучшие практики и правила @AGENTS.md" 
12. Просите добавить всю полезную информацию полученную при решении определенной задачи в @AGENTS.md для дальнейшего переиспользования, если приходится о чем-то просить агента более чем один раз это лучше зафиксировать
